...
int main(int argc, char **argv)
{
    ...
    result = hipfftPlan1d(&plan_z2z_fwd, nx, HIPFFT_Z2Z, 1);

    result = hipfftPlan1d(&plan_z2z_bwd, nx, HIPFFT_Z2Z, 1);
    ...
    for (int i = 0; i < nx; i++)
      {
        cx[i] = std::complex<double>(sin(i*dx),0.0);
        cy[i] = std::complex<double>(0.0,0.0);
      }
