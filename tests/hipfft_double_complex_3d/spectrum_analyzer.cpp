...
// complex input: analyzing all 3 dims for both D2Z and Z2Z transforms
void Get_Frequency_Spectrum(int IsD2Z, size_t inx, size_t iny, size_t inz, std::complex<double> sx[])
{
    ...
    double anx = double(inx);
    double any = double(iny);
    double anz = double(inz);
    double factor = 1.0/(anx*any*anz);

    double Fs;
    double mag_sx;

    double *spec_freq = NULL;
    double *spec_amp = NULL;

    spec_freq = (double*)malloc(3*max_indx*sizeof(double));
    spec_amp = (double*)malloc(3*max_indx*sizeof(double));
            ...
            for (i = 0; i < l_inx; i++)
              {
                ...
                // freq = double(i)*Fs/anx;
                spec_freq[i] += double(i);
              }
            x_counter += 1.0;
          }
      }
            ...
            for (j = 0; j < iny/2; j++)
              {
                ...
                // freq = double(i)*Fs/any;
                spec_freq[doffset + j] += double(j);
              }
            y_counter += 1.0;
          }
      }
            ...
            for (k = 0; k < inz/2; k++)
              {
                ...
                // freq = double(i)*Fs/axl;
                spec_freq[doffset + k] += double(k);
              }
            z_counter += 1.0;
          }
      }
    ...
    // post-process and clean up the data for the D2Z transforms
